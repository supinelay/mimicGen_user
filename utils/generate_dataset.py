...
" 修改 mimicGen 中的 generate_dataset"
...
def generate_dataset(
    ...
    new_dataset_path = os.path.join(new_dataset_folder_path, "lift.hdf5")
    ...
    use_image_obs = (
        (mg_config.obs.collect_obs and (len(mg_config.obs.camera_names) > 0)) if not write_video else False)
    use_depth_obs = False
    ...
    while True:
        ...
        if success:
            ...
            if mg_config.experiment.generation.keep_failed and \
                    (mg_config.experiment.max_num_failures is None) or (
                    num_failures <= mg_config.experiment.max_num_failures):
    ...
    if mg_config.experiment.render_video:
        if (num_success > 0):
            playback_video_path = os.path.join(new_dataset_folder_path,
                                               "playback_{}.mp4".format(new_dataset_folder_name))
        ...
        if mg_config.experiment.generation.keep_failed:
            if (num_failures > 0):
                playback_video_path = os.path.join(new_dataset_folder_path,
                                                   "playback_{}_failed.mp4".format(new_dataset_folder_name))
